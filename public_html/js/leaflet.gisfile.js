/*
 * L.GISFileJSON turns GISFile API box (http://gisfile.com/api/1.0/doc/jsonp/) data into a Leaflet layer.
*/
...
L.GISFileAPI = L.Class.extend({
    ...
    , setStyle: function (that) {
        ...
        if (that.options.onFinish) 
            that.onFinish = that.options.onFinish;
            ...
            , success: function(response) {
                ...
                if (response && response.data && that.process[p]) 
                {
                    ...
                    for (var i=0;i<data.length;i++) 
                    {
						...
						if (item.properties && item.properties.uid) {
