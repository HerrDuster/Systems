		...
		handlers: {
			...
			ruler: {
				tooltip: {
					...
					end: 'Click first point to finish route.'
				}
			},
			polyline: {
...
L.Draw.Polyline = L.Draw.Feature.extend({
		...
		if (this._map) {
			...
			this._map
				...
				.on('zoomend', this._onZoomEnd, this);
                        ...
                        L.DomEvent.on(this._container, 'keyup', this.cancelLastVertex, this);
		}
	},
	...
	removeHooks: function () {
		...
		this._map
			...
			.off('zoomend', this._onZoomEnd, this);
                ...
                L.DomEvent.off(this._container, 'keyup', this.cancelLastVertex, this);
	},

	cancelLastVertex: function (e) {
		if (e.keyCode === 46) {
	...
	_onMouseMove: function (e) {
		...
		if (typeof map.options.snapping !== "undefined" && map.options.snapping.enabled === true) 
                {
                    this._poly.options.snapping = map.options.snapping;
		...
		if (this._mouseDownOrigin) 
                {
                    ...
                    if (Math.abs(distance) < 9 * (window.devicePixelRatio || 1)) {
                            this.addVertex( latlng); //e.latlng);
                    }
		}
	...
	_createMarker: function (latlng) {
                ...
                if (markerCount < 1 || !this.options.showLabel) {
                    var marker = new L.Marker(latlng, {
                        icon: this.options.icon,
                        zIndexOffset: this.options.zIndexOffset * 2
                    });
                } else {
                    var marker = new L.Marker.Text(latlng, this._getLengthString(), {
                        icon: this.options.icon,
                        zIndexOffset: this.options.zIndexOffset * 2
                    });
                }
...
L.Draw.Polygon = L.Draw.Polyline.extend({
	...
	_getTooltipText: function () {
            ...
            if (this._markers.length === 0) {
                ...
                subtext = this._getAreaString();
            }

            if (this.options.showLength && this._markers.length > 0) {
                subtext = this._getLengthString() +(subtext != undefined ? " " +subtext : "");
            }
	...
	_getAreaString: function () {
            ...
            if (!this.options.allowIntersection && this.options.showArea) 
            {
                ...
                var arr = new Array()
                area = L.GeometryUtil.geodesicArea( arr.concat(latLngs).concat([currentLatLng]));
            }
...
L.Draw.Ruler = L.Draw.Polygon.extend({
	statics: {
		TYPE: 'ruler'
	},
	...
	_getTooltipText: function () {
            ...
            if (this._markers.length === 0) {
                text = L.drawLocal.draw.handlers.ruler.tooltip.start;
            } else if (this._markers.length < 2) {
                text = L.drawLocal.draw.handlers.ruler.tooltip.cont;
            } else {
                text = L.drawLocal.draw.handlers.ruler.tooltip.end;
                subtext = this._getAreaString();
            }
...
L.Marker.Text = L.Marker.extend({
        initialize: function (latlng, text, options) {
                ...
                this._txticon = new L.Icon.Text(text);
        },
        ...
        _initIcon: function() {
                ...
                this.options.icon = this._txticon;
                L.Marker.prototype._initIcon.apply(this);
                ...
                off.x = -10;
                if (L.Browser.webkit) off.y = +h/2;
                L.DomUtil.setPosition(i, off);
                //if (s) L.DomUtil.setPosition(s, off);
                //var txt = new L.Point(w/2, -h/2);
                //L.DomUtil.setPosition( obj, txt);
        }
});
